...
系统化重构版，包含 **18个分类**，顶部完整目录索引。主要特点：

- **设计令牌**：所有颜色、字体、间距使用CSS变量，修改一处全局生效
- **教学优化**：增强投影对比度、当前章节提示、练习题和代码块可读性
...
| `.tutorial-nav` | 侧边导航栏 | 固定左侧，宽度由 `--nav-width` 控制 |
| `.tutorial-content` | 主内容区域 | 左边距跟随 `--nav-width`，最大宽度由 `--content-max-width` 控制 |
...
| `.subsection` | 知识点块 | 一个section内的子分区，自动显示“知识点 N”并提供清晰边界 |
| `.subsection-title` | 知识点标题 | h3，24px，半粗600，带标题分隔线 |
